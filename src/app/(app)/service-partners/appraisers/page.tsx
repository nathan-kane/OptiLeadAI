...
export default function AppraisersPage() {
  const [appraiserCompanies, setAppraiserCompanies] = useState<AppraiserCompany[]>([]);
  ...
  useEffect(() => {
    if (user) {
      fetchAppraiserCompanies();
    }
  }, [user]);

  const fetchAppraiserCompanies = async () => {
    try {
      setLoading(true);
      ...
      const appraiserCompaniesRef = collection(db, 'users', user.uid, 'appraiserCompany');
      const q = query(appraiserCompaniesRef, orderBy('appraiserCompanyName'));
      const querySnapshot = await getDocs(q);
      
      const companies: AppraiserCompany[] = [];
      ...
      setAppraiserCompanies(companies);
    } catch (error) {
      console.error('Error fetching appraiser companies:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCompanyClick = (companyId: string) => {
    router.push(`/service-partners/appraisers/${companyId}`);
  };

  const handleAddNew = () => {
    router.push('/service-partners/appraisers/new');
  };
    ...
    <div className="container mx-auto p-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          <PageHeader
                  title="Appraisers"
                  description="Manage your appraiser partners."
                />
        </div>
      ...
      {appraiserCompanies.length === 0 ? (
        <Card className="shadow-xl border-0 bg-white rounded-2xl overflow-hidden">
          <CardContent className="flex flex-col items-center justify-center py-12 px-6">
            ...
            <h3 className="text-xl font-extrabold text-slate-900 mb-2">No appraisers found</h3>
            <p className="text-slate-600 text-center mb-6">
              Get started by adding your first appraiser partner.
            </p>
            <Button onClick={handleAddNew} className="flex items-center gap-2 rounded-full bg-gradient-to-r from-blue-600 to-green-600 text-white font-semibold uppercase hover:scale-105 transition-all duration-200">
              <Plus className="h-4 w-4" />
              Add First Appraiser
        ...
        <Card className="shadow-xl border-0 bg-white rounded-2xl overflow-hidden">
          <CardHeader className="px-2 sm:px-6 py-6 bg-gradient-to-r from-blue-50 to-green-50 border-b border-gray-100">
            <CardTitle className="text-2xl sm:text-3xl font-extrabold text-slate-900">Appraisers</CardTitle>
            <CardDescription className="text-base sm:text-lg text-slate-600">
              Showing {appraiserCompanies.length} appraiser partners. Manage your valuation network.
            </CardDescription>
          </CardHeader>
                  ...
                  <div className="space-y-2 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-lg font-extrabold text-slate-900 hover:text-blue-600 transition-colors">
                        {company.appraiserCompanyName || 'N/A'}
                      </h3>
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      ...
                      <span>
                        {company.city || 'N/A'}, {company.appraiserCompanyState || 'N/A'}
                      </span>
                    </div>
